...
しかし、Rの `urca` パッケージではADF検定の統計量tの期待値と分散 $E[t_i], var(t_i)$ を求める方法が分からなかった。  
そこで、パネルデータ分析パッケージ `plm` のパネル単位根検定用関数 `purtest()` を用いる。  
...
これはADF検定の統計量が小さい国を恣意的に除外し、IPS統計量 $Z_{\bar{t}}$ を無理やり大きくしたためである。  
